...
REPO_APP       ?= repo-cdk-hono-template-app
REPO_MIGRATE   ?= repo-cdk-hono-template-migrate
REPO_JUMP      ?= repo-cdk-hono-template-jump

DOCKERFILE_APP     ?= ./apps/app/Dockerfile.app
DOCKERFILE_MIGRATE ?= ./apps/app/Dockerfile.migrate
DOCKERFILE_JUMP   ?= ./apps/app/Dockerfile.jump
...
# ---- migrate image ----
build-migrate:
	docker build ./apps/app --platform $(PLATFORM) -f $(DOCKERFILE_MIGRATE) -t $(MIG_URI):$(TAG)
...
build-jump:
	docker build ./apps/app --platform $(PLATFORM) -f $(DOCKERFILE_JUMP) -t $(JUMP_URI):$(TAG)

push-jump:
	docker push $(JUMP_URI):$(TAG)

jump: login build-jump push-jump
